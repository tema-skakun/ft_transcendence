...
export const User = (props: blockProps) => {
  ...
  async function handleActivate2f () {
	...
	const url = 'http://localhost:3333/2fa/generate'; // replace with your API endpoint URL
	const headers = {
		Accept: 'image/png',
  ...
  const formSubmit = async (e: any) => {
	...
	if (activationCode) {
		const url = 'http://localhost:3333/2fa/turn-on'; // replace with your API endpoint URL
		const headers = {
	...
	if (profilePic || (username != prevUsername)) {
		const url = 'http://localhost:3333/users/update'; // replace with your API endpoint URL
		const headers = {
			'Content-Type': 'application/json', // set the appropriate Content-Type header
			'Authorization': `Bearer ${JSCookies.get('accessToken')}`,
		};
